...
class DefaultDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        subwords = pad_sequence(subwords, batch_first=True, padding_value=self.transform.tokenizer.pad_token_id)
        #tags = pad_sequence(
        #    tags, batch_first=True, padding_value=self.vocab.tags[0].get_stoi()["O"]#["O"] "<pad>"
        #)
        tags = pad_sequence(
            tags, batch_first=True, padding_value=self.vocab.tags[0].get_stoi()["O"]#["O"] "<pad>"
        )
        if subwords.shape != tags.shape:
            print(f'subwords {subwords.shape}')
...
class NestedTagsDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        tags = torch.cat(tags)
        #print('lbls dataset', tags.shape)
        #print(tags)
        #print('subwords dataset', subwords.shape)
