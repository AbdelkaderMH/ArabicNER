...
class BertMTNTagger(BaseModel):
    def __init__(self, **kwargs):
        super(BertMTNTagger, self).__init__(**kwargs)

        self.max_num_labels = max(self.num_labels)
        print(self.max_num_labels)
        print(self.num_labels)
        hidden_size = self.bert.config.hidden_size
        classifiers = [nn.Linear(hidden_size, self.max_num_labels) for num_labels in self.num_labels]
