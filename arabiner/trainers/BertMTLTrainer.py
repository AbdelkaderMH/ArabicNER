...
class BertMTLTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for epoch_index in range(self.max_epochs):
            ...
            for batch_index, (_, gold_tags, tokens, _, logits) in enumerate(self.tag(
                self.train_dataloader, is_train=True
            ), 1):
                
                self.current_timestep += 1
                batch_loss = self.criterion(logits.view(-1, logits.shape[-1]), gold_tags.view(-1).long())
                #mean = batch_loss.mean()
                #penalty = 5 * ((batch_loss - mean) ** 2).mean()
                #batch_loss = mean + penalty
                batch_loss.backward()
    ...
    def eval(self, dataloader):
        ...
        for _, gold_tags, tokens, valid_len, logits in self.tag(
            ...
            #gold_tags_v = gold_tags[valid_mask]
            #print('eval logits_v ', logits_v.shape)
            loss += self.criterion(logits.view(-1, logits.shape[-1]), gold_tags.view(-1))
            #print('eval logits_v ', logits_v.shape)
